"""
agents/remoteok_agent.py

Agent that pulls remote job listings from RemoteOK's public API,
validates them, writes to MongoDB, and publishes to the queue for
downstream processing (embedding/segmentation).
"""

from typing import Dict, Optional

from agents.base_agent import BaseAgent
from scrapers.remoteok import fetch_jobs
...
class RemoteOKAgent(BaseAgent):
    ...
    def __init__(self):
        super().__init__(agent_name="remoteok_agent")

    def run(self, limit: int = 50):
        self.logger.info(f"Starting RemoteOK fetch, limit={limit}")
        ...
        for job in jobs:
            ...
            try:
                ...
                self.queue.publish(
                    ...
                    message={"url": validated.get("url"), "source": "remoteok"},
                )
                inserted += 1
