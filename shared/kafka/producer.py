...
def _get_compression_type() -> str | None:
    """Return a Kafka compression codec that is available in this environment.

    ``aiokafka`` validates optional codecs while the producer is constructed.
    In particular, requesting LZ4 without the ``lz4`` package prevents every
    worker from starting.  Compression is a performance optimisation, not a
    correctness requirement, so use no compression when the requested optional
    codec is unavailable.
    """
    configured = os.getenv("KAFKA_COMPRESSION", "none").strip().lower()
    ...
    package_name = optional_codecs.get(configured)
    if package_name:
        # Delegate the compatibility check to aiokafka. This is stricter than
        # importing a package with the same name and exactly matches the
        # validation AIOKafkaProducer performs in its constructor.
        try:
            from aiokafka import codec as aiokafka_codec

            checker = getattr(aiokafka_codec, f"has_{configured}", None)
            codec_available = bool(checker and checker())
        except (ImportError, AttributeError):
            codec_available = False

        if not codec_available:
            logger.warning(
                "Kafka compression '%s' requested but package '%s' is not usable by aiokafka; "
                "starting without compression. Install it to re-enable compression.",
                configured,
                package_name,
            )
            return None

    supported_codecs = {"gzip", *optional_codecs}
    if configured not in supported_codecs:
        logger.warning(
            "Unknown Kafka compression '%s'; starting without compression.", configured
        )
        return None
...
class KafkaProducerClient:
    ...
    async def start(self) -> None:
        compression_type = _get_compression_type()
        ...
        logger.info(
            ...
            extra={"servers": self.bootstrap_servers, "compression": compression_type or "none"},
        )
